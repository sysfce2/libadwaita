    ...
    *found_accent_colors = TRUE;
    if (!g_strcmp0 (env, "blue")) {
      ...
      g_warning ("Invalid accent color %s (Expected one of: blue, teal, green,"
                 "yellow, orange, red, pink, purple, brown, slate)", env);
    }
  }
}
...
{
  g_return_val_if_fail (ADW_IS_SETTINGS (self), ADW_ACCENT_COLOR_DEFAULT);
...
adw_settings_end_override (AdwSettings *self)
{
  ...
  self->accent_color_override = ADW_ACCENT_COLOR_DEFAULT;
                                                     ...
                                                     gboolean     system_supports_accent_colors)
{
  ...
  if (!system_supports_accent_colors)
    adw_settings_override_accent_color (self, ADW_ACCENT_COLOR_DEFAULT);
